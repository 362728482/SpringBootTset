 ...
 *     <ul>
 *         可以传递的参数
 *             <li>name：存放到Session的名称，不传默认存储系统设定的值</li>
 *             <li>type：验证码的字符，可以传递的值为：1、中文验证码；2、数字字母组合；3、纯数字；4、纯字母；不传默认为字符</li>
 *     </ul>
 * </p>
 *
 *
 * */
    ...
    @RequestMapping({"/randCode.html"})
    public void verify(HttpServletRequest request, HttpServletResponse response,String name ,String type){
        String createTypeFlag = request.getParameter("createTypeFlag");
        //新建一个图片文件（生成在内存图片缓存区）
        BufferedImage bi = new BufferedImage(WIDTH, HEIGHT, BufferedImage.TYPE_INT_RGB);
        //拿到图片 Graphics类提供基本绘图方法，Graphics类提供基本的几何图形绘制方法，主要有：画线段、画矩形、画圆、画带颜色的图形、画椭圆、画圆弧、画多边形等
...
//        String baseNumLetter = "0123456789ABCDEFGHJKLMNOPQRSTUVWXYZ";
        String baseNumLetter = "0123456789abcdefghijkmnpqrstuvwxyz";
